...
function getFranchiseeInfo($order_no, &$actual_order_no, &$ordered_on, &$centre_name, &$address1, &$address2, &$address3, &$address4, &$address5, &$principle_name, &$principle_contact_no, &$operator_name, &$operator_contact_no, &$name, &$ic_no, &$courier, &$tracking_no, &$remarks)
{
   ...
   c.principle_contact_no, c.operator_name, c.operator_contact_no, o.name, o.ic_no, o.courier, o.tracking_no, o.remarks from centre c, `defective` o where o.centre_code=c.centre_code and sha1(o.order_no)='$order_no'";
   //echo $sql;
...
$company_sql = "SELECT c.code, c.description from `defective` o, product p, codes c 
      ...
      <div class="uk-margin-left uk-margin-right so-content" id="company-<?php echo $company_row['code'] ?>" <?php echo $active_company[0] == $company_row['code'] ? '' : 'style="display:none"' ?>>
            ...
            <tr>
               ...
               <td class="td-no-border" style="text-align:left;">
                  ...
                  <?php } elseif ($company_name == "Q-DEES HOLDINGS SDN. BHD.") { ?>
                     <img src="../images/Logo Qdees Scholar.png" alt="logo" style="width:150px;"><br>
         ...
         <table>
            ...
            <tbody>
               ...
               $sql = "SELECT o.*, p.product_name from `defective` o, product p where sha1(order_no)='$order_no' and p.product_code=o.product_code and p.category='$company_name' and (o.cancelled_by = '' OR o.cancelled_by IS NULL) order by id";
               //echo $sql;
               $result = mysqli_query($connection, $sql);
