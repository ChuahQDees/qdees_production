         ...
         <div class="uk-width-5-10">
            <table class="uk-table uk-table-small">
               <tr class="uk-text-small">
                  <td class="uk-width-3-10 uk-text-bold">Parent Category : </td>
                  <td class="uk-width-7-10">
                     <select name="parent" id="parent" class="uk-width-1-1">
                        <option value="">Select</option>
<?php
function isParentCategory($code) {
   global $connection;

   $sql="SELECT * from codes where code='$code' and module='CATEGORY' and parent=''";
...
function getParentCategory($code) {
   ...
   $sql="SELECT * from codes where code='$code' and module='CATEGORY'";
   $result=mysqli_query($connection, $sql);
   $row=mysqli_fetch_assoc($result);
   return $row["parent"];
}

function isWantedCategory($code) {
   global $connection;

   if (isParentCategory($code)) {
      return true;
   } else {
      $parent=getParentCategory($code);
...
$sql="SELECT * from codes where module='CATEGORY' order by code";
$result=mysqli_query($connection, $sql);
while ($row=mysqli_fetch_assoc($result)) {
   if (isWantedCategory($row["code"])) {
?>
                        <option value="<?php echo $row['code']?>" <?php if ($row['code']==$edit_row['parent']) {echo "selected";}?>><?php echo $row['code']?></option>
<?php
   }
}
?>
                     </select>
                     <div class="uk-text-muted">Consider as root category if left empty</div>
                  </td>
               </tr>
               <tr class="uk-text-small">
...
?>
      <tr class="uk-text-small">
         ...
         <td><?php echo $browse_row["parent"]?></td>
