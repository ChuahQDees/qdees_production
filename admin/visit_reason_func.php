...
//if (($_SESSION["isAdmin"]==1) & ($_SESSION["isLogin"]==1)) {
   ...
   if ($mode=="SAVE") {
      ...
      if (isRecordFound($table, $key_field, $key_value)) {
         if ($$p_module!="") {
            $value=$$p_module;
            $hidden_variable="hidden_".$p_module;
            $hidden_value=$$hidden_variable;
            $update_sql="UPDATE `$table` set code='$value', description='$description' where code='$hidden_value' and module='$module'";
         ...
         if ($$p_module!="") {
            $value=$$p_module;
            $insert_sql="INSERT into `$table` (code, description, module) values ('$value', '$description', '$module')";
