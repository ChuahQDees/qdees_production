...
if ($sOrderNo!="") {
   ...
   $sql="UPDATE `order` set finance_approved_by='".$_SESSION["UserName"]."', 
   finance_approved_on='".$datetime."' where sha1(order_no)='$sOrderNo'";
   $result=mysqli_query($connection, $sql);
    ...
    $n_data['is_hq_read'] = 1;
	$n_data['type'] = "order_finance_approved";
	$n_data['subject'] = "Finance approved for sales order no. " .$row['order_no']; 
	notification($n_data);
	// Notification end
   if ($result) {
