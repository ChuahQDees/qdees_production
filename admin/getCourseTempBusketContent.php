...
function calcTotal($student_code) {
   ...
   $sql="SELECT sum(qty*unit_price) as total from tmp_busket where session_id='$session_id' and student_code=sha1('$student_code') and (collection_type = 'placement' or collection_type = 'math' or collection_type = 'mandarin' or collection_type = 'Enhanced Foundation Mandarin' or collection_type = 'international' or collection_type = 'foundation' or collection_type = 'robotic-plus' or collection_type = 'pendidikan' or collection_type = 'product')";
   //echo $sql;
   $result=mysqli_query($connection, $sql);
...
?>
   ...
   <tr class="uk-text-small">
      ...
      <td><?php 
		 if($row["collection_type"]=="math"){
			 echo "IQ Math";
		 }else if($row["collection_type"]=="mandarin"){
			 echo"Mandarin";
		 }else if($row["collection_type"]=="international"){
			 echo "International Art";
		 }else if($row["collection_type"]=="foundation"){
			 echo "International English"; 
		 }else if($row["collection_type"]=="pendidikan"){
			 echo "Pendidikan Islam"; 
      }else if($row["collection_type"]=="robotic-plus"){
         echo "Robotic Plus"; 
		 }else{
