...
//if (($_SESSION["isAdmin"]==1) & ($_SESSION["isLogin"]==1)) {
   ...
   if ($mode=="SAVE") {
      ...
      if (isRecordFound($table, $key_field, $$p_module)) {
         if (($$p_module!="") & ($country!="") & ($use_code!="")) {
            $value=$$p_module;
            $hidden_variable="hidden_".$p_module;
            $hidden_value=$$hidden_variable;
            $update_sql="UPDATE `$table` set code='$value', description='$description', country='$country', use_code='$use_code'
         ...
         if (($$p_module!="") & ($country!="") & ($use_code!="")) {
            $value=$$p_module;
            $insert_sql="INSERT into `$table` (code, description, module, country, use_code) 
            values ('$value', '$description', '$module', '$country', '$use_code')";
   ...
   $str=$_GET["s"];
   if ($_GET["s"]!="") {
      $browse_sql="SELECT * from `$table` where (code like '%$str%' or country like '%$str%') and module='$module' order by code";
   } else {
