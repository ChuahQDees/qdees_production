...
function getPaidStatus($id) {
   ...
   if ($row["finance_payment_paid_by"]!="") {
      ...
      $sql="SELECT * from `user` where user_name='$finance_payment_paid_by' and user_type='S'";
      //echo $sql;
      $result=mysqli_query($connection, $sql);
      $row=mysqli_fetch_assoc($result);
      if ($row["id"]!="") {
...
function getPaidStatus1($order_no) {
   global $connection;
...
function doRejectCancelRequest(sha_id, sOrderNo) {
   //alert(sOrderNo);
   $.ajax({
      url : "admin/dlg_cancel.php",
      type : "POST",
      data : "sha_id="+sha_id+"&sOrderNo="+sOrderNo+"&reject=1",
      dataType : "text",
      beforeSend : function(http) {
      },
      success : function(response, status, http) {
         ...
         $("#dlg").dialog({
            ...
            height:'auto',
            width:'auto',
            title:"Please provide a reason to reject ",
         });
      },
      error : function(http, status, error) {
         UIkit.notify("Error:"+error);
...
function doApproveCancelRequest(id, sOrderNo)
{
   UIkit.modal.confirm("<h2>Are you sure you want to approve request?</h2>", function () {
   var reason=$("#reason").val();
   $.ajax({
      url : "admin/do_cancelled.php",
      type : "POST",
      data : "id="+id+"&sOrderNo="+sOrderNo+"&approve=1",
      dataType : "text",
      beforeSend : function(http) {
      },
      success : function(response, status, http) {
...
//if ($_SESSION["UserType"]=="S") {
  ...
  $sql21 ="SELECT *  FROM prove_payment_doc inner JOIN user ON prove_payment_doc.payment_by=user.user_name WHERE prove_payment_doc.order_no = '$sOrderNo' and user.user_type='S' ";
//}
if($_SESSION["UserType"] == "A"){
 	$c_sql="UPDATE `notifications` set is_center_read='1' where send_to='".$_SESSION['CentreCode']."' AND action_id='".$sOrderNo."'"; 
}else{
	$c_sql="UPDATE `notifications` set is_hq_read='1' where send_to='hq' AND action_id='".$sOrderNo."'"; 
}
...
//echo $d;
// $g=$row['remarks'];
$sql22 ="SELECT prove_payment_doc.*, prove_payment.remark  FROM prove_payment_doc left JOIN prove_payment ON prove_payment.id=prove_payment_doc.prove_payment_id WHERE prove_payment_doc.order_no = '$od' AND `prove_payment_doc`.`deleted` = '0' order by prove_payment.id desc";
...
echo "<td><button type='button' class='close' >
<span aria-hidden='true' onclick='delete_provePayment(".$row22['id'].",".$row22['prove_payment_id'].")'>&times;</span></button></td>";
echo "</tr>";
}
?>
  ...
  <div class="nice-form">
    <div class="uk-grid">
     ...
     <div class="uk-width-medium-5-10">
      ...
      // if (($row["cancelled_by"]=="") & ($_SESSION["isLogin"]==1) & (($_SESSION["UserType"]=="S") || ($_SESSION["UserType"]=="A"))) {
         if (($row["cancelled_by"]=="") & ($_SESSION["isLogin"]==1) & (($_SESSION["UserType"]=="S"))) {
      ?>
            <a onclick="doCancelOrder('', '<?php echo $sOrderNo?>')" class="uk-button uk-button-small form_btn text-white"><i class="fa fa-times-circle"></i> Cancel Order</a>
      <?php
      }
      ?>
      <br><br><br>
...
?>
         <th data-uk-tooltip="{pos:top}" title="Packed By">Checked by</th>
<?php
}
?>
<?php
...
<?php if ($_SESSION["UserType"]!="A") { ?>
   ...
   <th class="uk-text-center">Cancel Request By</th>
   <th class="uk-text-center">Request Reason</th>
<?php } else { ?>
   <th class="uk-text-center">Cancel Request</th>
   <th class="uk-text-center">Request Reason</th>
   <th class="uk-text-center">Reject Reason</th>
<?php } ?>
...
?>
 <tr class="">
		...
		//echo $row["product_code"]
		 ...
		 ?></td>
       <td><?php echo getCentreNameOS($row["centre_code"]) ?></td>
	   ...
	   <?php }else{ ?>
            ...
            echo $product_code;
            ?>
            <!-- <a href="index.php?p=defective_status&sOrderNo=<?php //echo $row['defective_no']?>"><?php //echo $row["defective_no"]?></a> -->
         </td>
...
if (hasRightGroupXOR($_SESSION["UserName"], "AssignedToEdit|AssignedToView")) {
?>
<td><div data-uk-tooltip="{pos:top}" title="<?php echo DateTime($row['assigned_to_by'], $row['assigned_to_on'])?>"><?php echo Person($row["checked_by"]).' (By '.Person($row["assigned_to_by"]).')'?></div></td>
<?php
}
?>
<?php
if (hasRightGroupXOR($_SESSION["UserName"], "PackedEdit|PackedView")) {
?>
         <td><div data-uk-tooltip="{pos:top}" title="<?php echo packedDateTime($row['id'], $row['packed_by'], $row['packed_on'])?>"><?php echo Person($row["packed_by"]).getPaidStatus1($row["order_no"])?></div></td>
...
if (hasRightGroupXOR($_SESSION["UserName"], "DeliveryEdit|DeliveryView")) {
?>
         <td><div data-uk-tooltip="{pos:top}" title="<?php echo deliveredToDateTime($row['id'], $row['delivered_to_logistic_by'], $row['delivered_to_logistic_on'])?>"><?php echo Person($row["delivered_to_logistic_by"]).getPaidStatus1($row["order_no"])?></div></td>
...
?>
   ...
   <td class="uk-text-center"><?php if($row['request_by'] != '') { echo $row["request_by"]; } ?></td>
   <td class="uk-text-center"><?php if($row['request_by'] != '') { echo $row["cancel_request_reason"]; } ?></td>

   <?php } else { ?> 

      <td class="uk-text-center"><?php if($row['request_by'] == $_SESSION["UserName"]) { if($row['approve_cancellation'] == '0') { echo 'Pending'; } else if($row['approve_cancellation'] == '1') { echo 'Approved'; } else if($row['approve_cancellation'] == '2') { echo 'Rejected'; } } ?></td>

      <td class="uk-text-center"><?php if($row['request_by'] == $_SESSION["UserName"]) { echo $row["cancel_request_reason"]; } ?></td>

      <td class="uk-text-center"><?php if($row['request_by'] == $_SESSION["UserName"]) { echo $row["request_reject_reason"]; } ?></td>
   
   <?php } ?>
 <td class="uk-text-center">
   <?php
      if($row['cancelled_by'] == '' && $row['request_by'] != '' && $row['request_reject_reason'] == '' && $_SESSION["UserType"]=='S')
      {
   ?>
         <a onclick="doApproveCancelRequest('<?php echo sha1($row['id']); ?>', '<?php echo $sOrderNo?>')" title="Approve Request" ><i style="color: #19a02b; font-size:15px;" class="fas fa-check"></i></a>

         <a onclick="doRejectCancelRequest('<?php echo sha1($row['id']); ?>', '<?php echo $sOrderNo?>')" title="Reject Request" ><i style="color: #000; font-size:15px;" class="fas fa-ban"></i></a>
   <?php
      }
      if (($row["cancelled_by"]=="") & ($_SESSION["isLogin"]==1) && ($row['request_by'] != $_SESSION["UserName"])) 
      {
   ?>
         <a onclick="doCancelOrder('<?php echo sha1($row['id']); ?>', '<?php echo $sOrderNo?>')" title="Request Cancellation" ><i style="color: #ff0000; font-size:15px;" class="fas fa-close"></i></a>
   <?php
      }
   ?>
 <?php
...
// });
      ...
      function delete_provePayment(id,prove_payment_id) {

         if(confirm("Are you sure you want to delete?")){
            $.ajax({
               url : "admin/delete_provePayment.php",
               type : "GET",
               data : "id="+id+"&prove_payment_id="+prove_payment_id,
               dataType : "text",
               beforeSend : function(http) {
               },
               success : function(response, status, http) {
