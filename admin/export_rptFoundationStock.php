...
if ($_SESSION["isLogin"]==1) {
    ...
    $filename = 'foundation_term'.$term.'_centre_stock_report_'.date('Y-m-d').'.csv';
    ...
    if($term == 1) {
        $content[] = array("","","EDP1","M1","M5","M9","EDP1","M1","M5","M9","EDP1","M1","M5","M9","EDP1","M1","M5","M9","EDP1","M1","M5","M9");
    } else if($term == 2) {
        $content[] = array("","","EDP2","M2","M6","M10","EDP2","M2","M6","M10","EDP2","M2","M6","M10","EDP2","M2","M6","M10","EDP2","M2","M6","M10");
    } else if($term == 3) {
        $content[] = array("","","EDP3","M3","M7","M11","EDP3","M3","M7","M11","EDP3","M3","M7","M11","EDP3","M3","M7","M11","EDP3","M3","M7","M11");
    } else if($term == 4) {
        $content[] = array("","","EDP4","M4","M8","M12","EDP4","M4","M8","M12","EDP4","M4","M8","M12","EDP4","M4","M8","M12","EDP4","M4","M8","M12");
    } 
    ...
    $opening_balance_total4 = 0; $stock_delivered_total4 = 0; $stock_consumed_total4 = 0; $closing_stock_total4 = 0;
    ...
    while($centre_row = mysqli_fetch_array($centre))
    {
        ...
        $sql="SELECT sum(qty) as qty from (";
        $sql .=" SELECT sum(qty) as qty from `order` where product_code like '$product_code4%' and delivered_to_logistic_on<'$term_start_date' and delivered_to_logistic_by<>'' and (cancelled_by='' OR cancelled_by IS NULL) and centre_code='$centre_code' ";
        $sql .=" UNION ALL ";
        $sql .="SELECT sum(qty) * -1 as qty from `collection` where product_code like '$product_code4%' and CAST(collection_date_time AS DATE)<'$term_start_date' and centre_code='$centre_code' and void=0";
        $sql .=" )ab ";
        ...
        if ($row["qty"]=="") {
            ...
            $opening_balance4 = round($row["qty"], 0);
            $opening_balance_total4 += $row["qty"];  
        }
        $excel_row[] = $opening_balance4;                    
        ...
        $sql="SELECT sum(qty) as qty from `order` where product_code like '$product_code4%' and (delivered_to_logistic_on BETWEEN '$term_start_date' AND '$term_end_date') and delivered_to_logistic_by<>'' and (cancelled_by='' OR cancelled_by IS NULL) and centre_code='$centre_code' ";
        ...
        if ($row["qty"]=="") {
            ...
            $stock_delivered4 = round($row["qty"], 0);
            $stock_delivered_total4 += $row["qty"];  
        }
        $excel_row[] = $stock_delivered4;
        ...
        $sql="SELECT sum(qty)  as qty, count(DISTINCT `student_code`) as student_number from `collection` where product_code like '$product_code4%' and (collection_date_time BETWEEN '$term_start_date' AND '$term_end_date') and centre_code='$centre_code' and void=0";
        ...
        if ($row["qty"]=="") {
            ...
            $stock_consumed4 = round($row["qty"], 0);
            $stock_consumed_total4 += $row["qty"]; 
        }
        $student_number4 = $row['student_number'];
        $excel_row[] = $stock_consumed4;
        ...
        $excel_row[] = $closing_stock4 = $opening_balance4 + $stock_delivered4 - $stock_consumed4;

        $closing_stock_total4 += $closing_stock4;
        ...
        $excel_row[] = $student_number4;
        $student_number_total4 += $student_number4;
    ...
    $excel_row[] = round($opening_balance_total1, 0);
    $excel_row[] = round($opening_balance_total2, 0);     
    $excel_row[] = round($opening_balance_total3, 0);
    $excel_row[] = round($opening_balance_total4, 0);
    $excel_row[] = round($stock_delivered_total1, 0);
    $excel_row[] = round($stock_delivered_total2, 0);
    $excel_row[] = round($stock_delivered_total3, 0);
    $excel_row[] = round($stock_delivered_total4, 0);
    $excel_row[] = round($stock_consumed_total1, 0);
    $excel_row[] = round($stock_consumed_total2, 0);
    $excel_row[] = round($stock_consumed_total3, 0);
    $excel_row[] = round($stock_consumed_total4, 0);
    $excel_row[] = round($closing_stock_total1, 0);
    $excel_row[] = round($closing_stock_total2, 0);
    $excel_row[] = round($closing_stock_total3, 0);
    $excel_row[] = round($closing_stock_total4, 0);
    $excel_row[] = round($student_number_total1, 0);
    $excel_row[] = round($student_number_total2, 0);
    $excel_row[] = round($student_number_total3, 0);
    $excel_row[] = round($student_number_total4, 0);
