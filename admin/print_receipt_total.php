...
function getStudentInfoByStudentCode($student_code, &$student_name, &$classes) {
   global $connection;
   ...
   while($row=mysqli_fetch_assoc($result)){
      ...
      $group = $row["group_no"];
      $course_group[] = $course_name . ' (Group ' . $group . ')';
   }
...
function getCentreInfo($centre_code, &$centre_name, &$company_name, &$address1, &$address2, &$address3, &$address4, &$address5, &$principle_name, &$principle_contact_no) {
   global $connection;

   $sql="SELECT * from centre where centre_code='$centre_code'";
   $result=mysqli_query($connection, $sql);
   $row=mysqli_fetch_assoc($result);
   $centre_name=$row["kindergarten_name"];
...
function getCollectionInfo($batch_no, &$collection_date, &$centre_code, &$plain_batch_no, &$ref_no, &$cheque_no, &$payment_method, &$student_code, &$bank, &$remark) {
   global $connection;
...
if ($batch_no!="") {
   getCollectionInfo($batch_no, $collection_date, $centre_code, $plain_batch_no, $ref_no, $cheque_no, $payment_method, $student_code, $bank, $remark);
   getCentreInfo($centre_code, $centre_name, $company_name, $address1, $address2, $address3, $address4, $address5, $principle_name, $principle_contact_no);
   getStudentInfoByStudentCode($student_code, $student_name, $classes);
?>
<style>
table {
   width :100%;
...
<div class="uk-margin uk-margin-top uk-margin-left uk-margin-right">
   ...
   <table  >
      <tr>
        ...
        <?php echo $centre_name?>  - <?php echo getAOCentreFranciseeCompanyNOByCenterCode($centre_code)?><br>
   ...
   <table>
      <tr>
         ...
         <td class="uk-text-bold td-border" style="text-align: center;">PARTICULARS</td> 
...
<?php
   $sql="SELECT * from collection c inner join payment_detail p on p.id=c.payment_detail_id where sha1(c.batch_no)='$batch_no' and `void`=0  order by c.id asc"; // new
...
?>
      <tr>
         <td class="td-border" style="text-align: center;">1</td>
         <td class=" td-border" style="padding-left: 20px; border-right: 0;"><?php echo $title_description; ?> &nbsp;</td>
         <td class="uk-text-right td-border "><?php echo number_format($total_amount, 2)?></td>
      </tr>
<?php
?>
   </table>
...
<tr><h3 class="uk-text" style="margin-bottom: 0;margin-top:0px;">Note :</h3>
  ...
  <p class="uk-text-bold">ALL CASH AND CHEQUES MUST BE MADE PAYABLE TO :(<?php echo $centre_name?>  - <?php echo getAOCentreFranciseeCompanyNOByCenterCode($centre_code)?>)</p></td>
