...
function generateReport(method) {
   ...
   if (centre_code!="") {
      if (method=="screen" || method=="export") {
         $.ajax({
            url : "admin/a_rptMasterSubjectEnroll.php",
            type : "POST",
            data : "centre_code="+centre_code+"&centre_status="+centre_status+"&from_date="+from_date+"&to_date="+to_date+"&course_subject="+course_subject+"&selected_year="+selected_year,
            dataType : "text",
         ...
         $("#frmCentreStatus").val(centre_status);
         $("#frmPrint").submit()
      }
   } else {
...
<?php
$sql2 = "SELECT * from centre_status order by id";
...
<div class="uk-width-medium-1-10">
   ...
   <!--<div style="width: 15%;" class="uk-width-medium-2-10">
	...
	<div class="uk-width-medium-4-10">
      ...
      <button onclick="generateReport('export')" id="btnGenerate" class="uk-button">Export to CSV</button>
	</div>
...
<form id="frmPrint" action="admin/a_rptMasterSubjectEnroll.php" method="post" target="_blank" style="background: none!important; box-shadow: none!important; padding: 0;" >
   ...
   <input type="hidden" id="frmCentreStatus" name="centre_status" value="">
</form>
<style>
.uk-grid > * {
     padding-right: 0px;
...
<script src="https://www.jqueryscript.net/demo/table-csv-converter/table2csv.js" type="text/javascript"></script>
...
function exportReport() {
   $(".rtable").table2csv({
      file_name: 'master_subject_enrl.csv',
      header_body_space: 0
    });
    //var html = $(".rtable").outerHTML;
	//export_table_to_csv(html, "master_subject_enrl.csv");
};
