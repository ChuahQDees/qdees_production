   ...
   if ($mode=="SAVE") {
      ...
      if (isRecordFound($table, $key_field, $$p_module."|".$module."|".$parent)) {
         if ($$p_module!="") {
            ...
            $update_sql="UPDATE `$table` set code='$value', parent='$parent', description='$description' 
         ...
         $value=$$p_module;
         $insert_sql="INSERT into `$table` (`code`, parent, `description`, `module`) values ('$value', '$parent', '$description', '$module')";
         echo $insert_sql;
//         $msg=$insert_sql;
         if ($$p_module!="") {
            $value=$$p_module;
